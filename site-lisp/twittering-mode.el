...
;; Custom API URL
(defvar twittering-host-url   "twitter.com")
(defvar twittering-api-url    "api.twitter.com")
(defvar twittering-search-url "search.twitter.com")
...
(defvar twittering-unread-tweets-count 0)

(defvar twittering-unread-mode-line-string ""
  "Show number of unread tweets on the mode line.

You may want to add this to `global-mode-string'.")

(defvar twittering-invoke-buffer nil
  "Buffer where call of `twittering-get-and-render-timeline' happens.

This will be used for determining whether we should show unread
count on the mode line. ")

(defvar twittering-new-tweets-hook '(twittering-update-unread-mode-line)
...
(defvar twittering-reverse-mode nil
  "*Non-nil means tweets are aligned in reverse order of `twittering-host-url'.")
...
(defvar twittering-fill-prefix "       "
  "Prefix used together with %FILL.

See also `twittering-status-format'." )
...
(defvar twittering-time-format nil
...
(defun twittering-get-status-url (username &optional id)
  "Generate status URL."
  (if id
      (format "http://%s/%s/statuses/%s" twittering-host-url username id)
    (format "http://%s/%s" twittering-host-url username)))
...
(defun twittering-update-unread-mode-line ()
  (if (or (twittering-buffer-p)
	  (twittering-buffer-p twittering-invoke-buffer))
      (setq twittering-unread-mode-line-string ""
	    twittering-unread-tweets-count 0)
    (when (> twittering-new-tweets-count 0)
      (setq twittering-unread-tweets-count
	    (+ twittering-unread-tweets-count twittering-new-tweets-count))
      (setq twittering-unread-mode-line-string
	    (format "tw(%d)" twittering-unread-tweets-count))))
  (force-mode-line-update))
...
(defun twittering-fill-string (str &optional adjustment)
  ...
  (let* ((kinsoku-limit 1)
    ...
    (with-temp-buffer
      (let ((fill-column (or twittering-fill-column temporary-fill-column))
	    (fill-prefix twittering-fill-prefix))
	(insert fill-prefix)
	(insert str)
...
(defun twittering-make-passed-time-string
  ...
  (let* ((now (current-time))
	 ...
	 (time-string
	  (cond
	   (twittering-time-format
	    (format-time-string twittering-time-format
				encoded-created-at))
  ...
  (if (twittering-timeline-spec-primary-p spec)
	...
	(cond
	 ...
	 ((eq type 'direct_messages)
	  `(,twittering-api-url "1/direct_messages"))
	 ((eq type 'direct_messages_sent)
	  `(,twittering-api-url "1/direct_messages/sent"))
	 ((eq type 'friends)
	  `(,twittering-api-url "1/statuses/friends_timeline"))
	 ((eq type 'home)
	  `(,twittering-api-url "1/statuses/home_timeline"))
	 ((eq type 'mentions)
	  `(,twittering-api-url "1/statuses/mentions"))
	 ((eq type 'public)
	  `(,twittering-api-url "1/statuses/public_timeline"))
	 ((eq type 'replies)
	  `(,twittering-api-url "1/statuses/replies"))
	 ((eq type 'retweeted_by_me)
	  `(,twittering-api-url "1/statuses/retweeted_by_me"))
	 ((eq type 'retweeted_to_me)
	  `(,twittering-api-url "1/statuses/retweeted_to_me"))
	 ((eq type 'retweets_of_me)
	  `(,twittering-api-url "1/statuses/retweets_of_me"))
	 ((eq type 'search)
	  (let ((word (car value)))
	    `(,twittering-search-url "search" ,word)))
	 (t
...
(defun twittering-host-method-to-timeline-spec (host method &optional word)
  (cond
   ...
   ((string= host twittering-api-url)
    (cond
	...
	`(list ,username ,listname)))
     (t nil)))
   ((string= host twittering-search-url)
    `(search ,word))
   (t nil)))
...
(defun twittering-mode-setup ()
  ...
  (add-to-list 'global-mode-string 'twittering-unread-mode-line-string t)
  (add-hook 'window-configuration-change-hook 'twittering-update-unread-mode-line)
  (twittering-update-mode-line)
  ...
  (let ((status (twittering-edit-extract-status))
	...
	(if username
	    (let ((parameters `(("user" . ,username)
				("text" . ,status))))
	      (twittering-http-post twittering-api-url
	...
	(let ((parameters `(("status" . ,status))))
	  ...
	  (twittering-http-post twittering-api-url "1/statuses/update"
				parameters))))
...
(defun twittering-start-http-session-curl (method headers host port path parameters &optional noninteractive sentinel)
  ...
  (let* ((request (twittering-make-http-request
	 ...
	 (headers
	  ;; FIXME: Looks like with custom API url, curl doesn't want
	  ;; us to set `Host:'? (xwl)
	  (remove-if (lambda (i)
		       (string= (car i) "Host"))
...
(defun twittering-http-post (host method &optional parameters format sentinel)
  "Send HTTP POST request to `twittering-api-url' (or `twittering-search-url').

HOST is hostname of remote side, `twittering-api-url' (or `twittering-search-url').
...
(defun twittering-get-list-index (username)
  (twittering-http-get twittering-api-url
...
(defun twittering-update-lambda ()
  ...
  (when (and (string= "Japanese" current-language-environment)
    ...
    (twittering-http-post
     twittering-api-url "1/statuses/update"
     `(("status" . ,(mapconcat
		     'char-to-string
